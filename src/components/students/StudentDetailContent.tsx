...
function StudentDetailInner({ portal = 'admin' }: { portal?: PortalId }) {
  ...
  const { data: beltHistory = [], isLoading: isHistoryLoading } = useBeltHistory(studentId);
  ...
  const closeEditModal = () => {
    ...
    if (searchParams.get('edit') === '1') {
      router.replace(`${studentsBasePath}/${studentId}`);
    }
  };
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    if (isInstructor && !formData.dojoId) {
  ...
  const handlePermanentDelete = () => {
    ...
    permanentlyDeleteStudent.mutate(student._id, {
      onSuccess: () => router.push(studentsBasePath),
    });
  };
  ...
  return (
    <div className="space-y-6 animate-fadeIn pb-8">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        {/* belt history (read-only for instructors) */}
        ...
        <div className="fixed inset-0 w-full h-full flex items-end sm:items-center justify-center p-0 sm:p-4 z-50 animate-fadeIn overflow-y-auto sm:overflow-visible">
          ...
          <div className="w-full sm:max-w-lg bg-zinc-950 border border-white/[0.08] rounded-t-2xl sm:rounded-2xl p-5 sm:p-8 shadow-[0_32px_64px_rgba(0,0,0,0.8)] z-10 relative sm:max-h-[92dvh] sm:overflow-y-auto pb-[max(1.25rem,env(safe-area-inset-bottom))]">
            ...
            <form onSubmit={handleFormSubmit} className="space-y-5">
              ...
              <DojoSelect
                label={isInstructor ? 'Dojo Branch (required)' : 'Dojo Branch'}
                value={formData.dojoId}
                onChange={(val) => setFormData({ ...formData, dojoId: val })}
                options={dojoOptions}
                placeholder={isInstructor ? 'Select your dojo...' : 'Select a dojo...'}
              />
              {selectedEditDojo && (
...
export function StudentDetailContent({ portal = 'admin' }: { portal?: PortalId }) {
  return (
    <Suspense fallback={<SkeletonPage />}>
      <StudentDetailInner portal={portal} />
    </Suspense>
  );
}
