...
function StudentsListInner({ portal = 'admin' }: { portal?: PortalId }) {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const studentsBasePath = usePortalPath(portal, '/students');
  ...
  const isInstructor = portal === 'instructor' || currentUser?.role === 'instructor';
  ...
  const {
    ...
    ...(isAdmin && selectedInstructor ? { instructor: selectedInstructor } : {}),
    ...
    <div className="h-[calc(100dvh-5rem)] sm:h-[calc(100dvh-7rem)] lg:h-[calc(100dvh-8rem)] min-h-0 flex flex-col gap-6 animate-fadeIn">
      ...
      <div className="shrink-0 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-white/[0.04] pb-6">
        <div>
          ...
          <p className="text-xs text-zinc-500 mt-1">
            {isInstructor
              ? 'View and manage students enrolled in your assigned dojos.'
              : 'Manage enrolled student records and belt assignments.'}
          </p>
      ...
      <div className="shrink-0 flex flex-col gap-3">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:flex md:flex-row gap-3">
          ...
          {(inputValue || selectedDojoId || selectedBelt || (isAdmin && selectedInstructor) || selectedCreatedBy) && (
            <button
              onClick={() => {
                setInputValue('');
                setParams({
                    ...
                    {students.map((student, index) => (
                      ...
                      >
                        {student._id === '__optimistic__' ? (
                          ...
                          <Link
                            href={`${studentsBasePath}/${student._id}`}
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              onClick={() => router.push(`${studentsBasePath}/${student._id}?edit=1`)}
      ...
      <AddStudentModal
        ...
        dojos={dojos}
        requireDojo={isInstructor}
      />
    </div>
  );
}

export function StudentsListContent({ portal = 'admin' }: { portal?: PortalId }) {
  return (
    <Suspense fallback={<SkeletonRows />}>
      <StudentsListInner portal={portal} />
    </Suspense>
  );
}
